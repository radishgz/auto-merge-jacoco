...
public class JavaCoverageLoader {
  ...
  private ISessionListener sessionListener = new ISessionListener() {
    ...
    public void sessionActivated(ICoverageSession session) {
      ...
      if (session == null) {
        ...
        ConsoleMessage.showMessage("Session Add");

        CoverageSession coveragesession = (CoverageSession) session;
        // .getActiveSession();

        ConsoleMessage.showMessage("DataSource..."
            + coveragesession.getExecutionDataSource().getClass().toString());
        URLExecutionDataSource dataSource = (URLExecutionDataSource) coveragesession
            .getExecutionDataSource();
        ConsoleMessage.showMessage("DataSource..."
            + dataSource.getUrl().toExternalForm());
  ...
  private class LoadSessionJob extends Job {
    ...
    protected IStatus run(IProgressMonitor monitor) {
      ...
      try {
        ...
        ConsoleMessage.showMessage("Session SessionAnalyzer Add");
  ...
  protected void fireCoverageChanged() {
    ...
    for (IJavaCoverageListener l : new ArrayList<IJavaCoverageListener>(
        listeners)) {
      ConsoleMessage.showMessage("Session fireCoverageChanged");
      l.coverageChanged();
    }
  }
